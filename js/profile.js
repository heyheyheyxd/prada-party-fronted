window.profilePage = function () {
    return {
        ...
        formatPhoneInput() {
            ...
            let core = digits.substring(1);
            let formatted = "8";

            if (core.length >= 1) formatted += "-" + core.substring(0, 3);
        ...
        formatPostalInput() {
            let digits = String(this.postal_code || "").replace(/\D/g, "");
            if (digits.length > 6) digits = digits.substring(0, 6);
            this.postal_code = digits;
        },
                ...
                input.addEventListener("keyup", (e) => {
                    if (e.key === "Enter") {
                        ...
                        window.location.href = q
                            ? `catalog.html?search=${encodeURIComponent(q)}`
                            : `catalog.html`;
                    }
                });
            }, 200);
        ...
        async saveInfo() {
            ...
            let pc = String(this.postal_code || "").replace(/\D/g, "");
